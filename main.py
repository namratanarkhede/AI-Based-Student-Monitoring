...
window = tk.Tk()
window.title("AI-Based Student Monitoring")

window.geometry('1280x720')
window.configure(background='gray85')

# GUI for manually fill attendance

def fill_manually():
    os.system('start excel "C:/Users/Namrata Narkhede/PycharmProjects/AI-Based Student Monitoring/Attendance_management/Attendance/Manually Attendance/Manually_fill_attendance.xlsx"')
...
def err_screen():
    ...
    sc1.title('Warning!!')
    sc1.configure(background='gray85')
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                subject_dir = os.path.join(
                    "C:/Users/Namrata Narkhede/PycharmProjects/AI-Based Student Monitoring/Attendance_management/Attendance",
                    Subject)
                if not os.path.exists(subject_dir):
                    os.makedirs(subject_dir)

                fileName = os.path.join(subject_dir,
                                        Subject + "_" + date + "_" + Hour + "-" + Minute + "-" + Second + ".csv")

                #fileName = "C:/Users/Namrata Narkhede/PycharmProjects/AI-Based Student Monitoring/Attendance_management/Attendance/" + Subject + "_" + date + \
                ...
                try:
                    ...
                    connection = pymysql.connect(
                        host='localhost', user='root', password='', db='Face_reco_fill', port=3307)
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"C:/Users/Namrata Narkhede/PycharmProjects/AI-Based Student Monitoring/Attendance_management/Attendance/-------Check atttendance-------"')
    ...
    fill_a = tk.Button(windo, text="Fill Attendance", fg="white", command=Fillattendances, bg="turquoise1", width=20, height=2,
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'admin':
            if password == 'admin123':
                ...
                cs = 'C:/Users/Namrata Narkhede/PycharmProjects/AI-Based Student Monitoring/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
    ...
    un = tk.Label(win, text="Enter username : ", width=15, height=2, fg="black", bg="gray85",
                  font=('times', 15, ' bold '))
    un.place(x=30, y=50)

    pw = tk.Label(win, text="Enter password : ", width=15, height=2, fg="black", bg="gray85",
                  font=('times', 15, ' bold '))
    pw.place(x=30, y=150)
    ...
    Login = tk.Button(win, text="LogIn", fg="black", bg="turquoise1", width=20,
                      height=2,
...
def getImagesAndLabels(path):
    ...
    Ids = []
    # now looping through all the image paths and loading the Ids and the static
...
message = tk.Label(window, text="AI-Based Student Monitoring", bg="turquoise4", fg="white", width=45,
...
lbl = tk.Label(window, text="Enter Enrollment : ", width=20, height=2,
               fg="black", bg="misty rose", font=('times', 15, 'bold'))
...
lbl2 = tk.Label(window, text="Enter Name : ", width=20, fg="black",
                bg="misty rose", height=2, font=('times', 15, ' bold '))
...
clearButton = tk.Button(window, text="Clear", command=clear, fg="white", bg="cyan4",
                        width=10, height=1, activebackground="white", font=('times', 15, ' bold '))
clearButton.place(x=950, y=210)

clearButton1 = tk.Button(window, text="Clear", command=clear1, fg="white", bg="cyan4",
                         width=10, height=1, activebackground="white", font=('times', 15, ' bold '))
clearButton1.place(x=950, y=310)

AP = tk.Button(window, text="Check Registered students", command=admin_panel, fg="black",
               bg="turquoise1", width=19, height=1, activebackground="white", font=('times', 15, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, text="Take Images", command=take_img, fg="black", bg="turquoise1",
                    width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images", fg="black", command=trainimg, bg="turquoise1",
                     width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendance", fg="black", command=subjectchoose,
               bg="turquoise1", width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manually Fill Attendance", command=fill_manually, fg="black",
                       bg="turquoise1", width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
